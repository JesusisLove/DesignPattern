...
public class StartMainCls {
    public static void main(String[] args) {
        ...
        /* 根据执行结果：sheep0，sheep1...这些克隆对象的friend全部都是一个对象：这样的拷贝就是浅拷贝：
        ...
        浅拷贝的形象描述就是：克隆羊虽然是10只不同的羊，但是这10只克隆羊却共用一个心肝肾肠胃。（心肝肾肠胃是内部的类成员变量）
        但是在实际开发中，我们也希望在执行克隆对象的时候，它里面的类成员变量也是希望是克隆对象里面自己的类对象成员变量，不要都指向同一个类成员变量
        这就会涉及到另外一个概念：深拷贝
        */
